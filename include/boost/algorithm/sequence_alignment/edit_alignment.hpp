...
template <typename Sequence1, typename Sequence2, typename Output, typename Cost, typename Beam, typename AllowSub, typename PruneBias>
...
edit_alignment_check(Sequence1 const& seq1, Sequence2 const& seq2, Output& output, const Cost& cost, const Beam& beam, const AllowSub& allow_sub, const PruneBias& prune_bias) {
    ...
    return dijkstra_sssp_alignment(boost::as_literal(seq1), boost::as_literal(seq2), output, cost, beam, allow_sub, prune_bias);
...
{
    return edit_alignment_check(seq1, seq2, output, cost, beam, allow_sub, prune_bias);
}
